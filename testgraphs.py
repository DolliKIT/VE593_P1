...
class nPuzzleGraph:
    ...
    def __init__(self, n):
        self.n = n
        Action.update(n)
        self.endState = list(range(1, n * n))
        self.endState.append(0)
...
class nPuzzleGraphValued(nPuzzleGraph):
    ...
    def successors(self, state):
        ...
        if (state.blankPosition % self.n != 0):
            insert = (1, self.succ(state, Action.LEFT))
            succs.append(insert)
        if (state.blankPosition >= self.n):
            insert = (1, self.succ(state, Action.UP))
            succs.append(insert)
        if (state.blankPosition % self.n != self.n - 1):
            insert = (1, self.succ(state, Action.RIGHT))
            succs.append(insert)
        if (state.blankPosition < self.n * (self.n - 1)):
            insert = (1, self.succ(state, Action.DOWN))
            succs.append(insert)
        return succs